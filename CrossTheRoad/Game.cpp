...
string Game::getNewPlayerName() {
	...
	console.gotoXY(newNameX, newNameY + 2);
	cout << "Player name: ";

	console.setTextColor(colorBlack);
	string name = "";
	while (name.length() < 1) {
...
void Game::drawMenu(int x, int y, int choice) {
	string menu[maxMenuChoice] = { "1. Start game","2. Load game","3. Setting", "4. Clear score board", "5. Tutorial", "6. Exit" };
...
void Game::drawLogo(int x, int y) {
	//Draw logo
	console.setTextColor(colorBlue);
...
void Game::drawObject() {
	...
	if (minRow == levelRow - nLanes) {
		vector<string> finish(sizeOfLanes, string(gameWindowX*3/4-1, char(95)));
...
void Game::drawInfo() {
	...
	console.setTextColor(colorBlack);
	fstream info("Tutorial.txt", ios::in);
	console.drawTextFromFile(info, infoX + 2, infoY);
	info.close();
